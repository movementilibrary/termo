...
public class TermOfUseService {
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(TermOfUseService.class);
	...
	/**
	 * Metodo responsável por vertificar se Flag is marcada, caso esteja marcada
	 * gera Versao Grande caso não estaje marcada gera versão menor
	 *
	 * @param
	 * @return
	 */
	...
	public String generatedNewSubVersion() {
		...
		return "V".concat(currentVersion.getVersion().toString()).concat(".")
				.concat(newSubVersion.getSubVersion().toString());
	}
