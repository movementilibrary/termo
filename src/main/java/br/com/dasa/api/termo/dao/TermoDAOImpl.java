...
public class TermoDAOImpl extends BaseDAO implements TermoDAO {
	...
	public Optional<TermOfUser> buscarUltimoTermoObrigatorio() {
		...
		jpql.append(
				"select t from TermOfUser t where t.flagAtualizacao = false and t.status = :status order by t.id desc ");
	...
	public List<TermOfUser> buscarTermosUltimosTermosNaoObrigatorios(Long idUltimoTermo) {
		...
		jpql.append(
				"select t from TermOfUser t where t.flagAtualizacao = true and t.status = :status and t.id > :idUltimoTermo order by t.id desc ");
	...
	@Override
	public TermOfUser buscarUltimoTermoAtivo() {

		try {

			StringBuilder jpql = new StringBuilder();

			jpql.append("select t from TermOfUser t where t.status = :status order by t.id desc ");

			Query query = em.createQuery(jpql.toString(), TermOfUser.class);

			query.setParameter("status", StatusTermUse.ACTIVE);

			query.setMaxResults(1);

			TermOfUser term = (TermOfUser) query.getSingleResult();
